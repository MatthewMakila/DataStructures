 ...
 *  fleet.h
 *  3/27/22
 *  This header file includes the definition of all class methods as well as
 *  the additional helper recursive functions used.
 */
#ifndef FLEET_H
#define FLEET_H
#include <iostream>
...
class Fleet{
    ...
    // ***************************************************
    Ship* insertHelper(Ship* root, const Ship& curr); // insertion recursive helper
    void clearHelp(Ship *pShip); // destructor recursive helper
    void inOrder(Ship *pShip) const; // helper to print out in order
    bool findHelper(Ship* aShip, int shipID) const;
    bool setStateHelp(int id, STATE state, Ship *aShip);
    Ship* removeHelper(Ship* aShip, int shipID);
    void removeLostHelp(Ship* aShip);
    Ship *RightRotateHelp(Ship *aShip);
    Ship *LeftRotateHelp(Ship *aShip);
    bool isLost(Ship *aShip);
};
#endif