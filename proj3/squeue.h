...
class SQueue{
    ...
    void clearHelp(Node *aNode) const; // helper for destructor
    Node* mergeHelper(Node *p1, Node *p2); // helper for merging new posts in
    void printPostsQueueHelp(Node *pNode) const; // helper for printing posts
    Node *copyHelper(Node *pNode); // helper for copy cons
};
#endif
